...
contract CollateralVault is ICollateralVault, AccessControl {
    ...
    modifier onlyAllowed() {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender) || hasRole(VAULT_ADMIN_ROLE, msg.sender), "Not Allowed");
        _;  
    }

    /// @dev Restrict to the wired Clearinghouse.
    modifier onlyClearingHouse() {
        require(msg.sender == clearinghouse, "Not Allowed");
        _;
    }
    ...
    /// @dev If the token already exists it will be overwritten and re-emitted.
    ...
    function deposit(address token, uint256 amount, address onBehalfOf) external onlyClearingHouse override returns (uint256 received) {
        ...
        require(!cfg.depositPaused, "Deposits paused");
        require(amount > 0, "Amount zero");
        require(onBehalfOf != address(0), "Zero address");
        ...
        if (cfg.accountCap != 0) {
            require(userBalances[onBehalfOf][token] + received <= cfg.accountCap, "Account cap");
        }
        if (cfg.cap != 0) {
            require(IERC20(token).balanceOf(address(this)) <= cfg.cap, "Token cap");
        }
    ...
    function withdrawFor(address user, address token, uint256 amount, address to) external onlyClearingHouse override {
        require(!collateralConfigs[token].withdrawPaused, "Withdrawals paused");
        require(amount > 0, "Amount zero");
        require(user != address(0), "Zero address");
        require(userBalances[user][token] >= amount, "Insufficient balance");
        userBalances[user][token] -= amount;
        IERC20(token).safeTransfer(to, amount);
        emit Withdraw(msg.sender, user, token, amount, to);
    }
    ...
    /// @dev Minimal implementation: simply transfers out up to amount from contract balance.
    /// Implementations may keep separate accounting for fees; this function is a thin shim.
    function sweepFees(address token, address to, uint256 amount) external onlyClearingHouse override {
        // Intentionally left minimal; implementation may track protocol fees separately.
        // For now, just transfer out from contract balance up to amount.
        if (amount > 0) {
            IERC20(token).safeTransfer(to, amount);
        }
    ...
    function creditFromPool(address pool, address token, address user, uint256 amount) external onlyClearingHouse override {
        ...
        if (cfg.accountCap != 0) {
            require(userBalances[user][token] + amount <= cfg.accountCap, "Account cap");
        }
