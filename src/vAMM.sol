...
contract vAMM is Initializable, UUPSUpgradeable, IVAMM {
	...
	/// @notice Buy base (receive base) by paying quote; slippage uses max acceptable price.
	...
	/// @notice Pay quote to receive base; slippage uses max acceptable price.
	...
	/// @param feeBps_ New trade fee in basis points (max 1%).
	...
	function setParams(
		...
		require(feeBps_ <= 100, "Fee too high"); // Max 1%
		feeBps = feeBps_;
		frMaxBpsPerHour = frMaxBpsPerHour_;
		kFundingX18 = kFundingX18_;
		observationWindow = observationWindow_;
